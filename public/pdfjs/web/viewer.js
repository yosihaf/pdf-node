...
const PDFViewerApplication = {
  ...
  setTitleUsingUrl(url = "", download_url = null) {
    this.url = url;
    this.baseUrl = url.split("#")[0];
    if (download_url) {
      this._download_url = download_url === url ? this.baseUrl : download_url.split("#")[0];
    }
  ...
  async download(options = {}) {
    ...
    try {
      ...
      await this.downloadManager.download_url(url, filename, options);
    }
  },
  async save(options = {}) {
...
class DownloadManager {
  ...
  download_url(url, filename, _options) {
    if (!(0, _pdfjsLib.createValidAbsoluteUrl)(url, "http://example.com")) {
      console.error(`download_url - not a valid URL: ${url}`);
      return;
    }
